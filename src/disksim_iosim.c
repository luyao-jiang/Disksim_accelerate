...
static int iosim_load_map(struct lp_block *b, int n) {
  ...
  lp_loadparams((void*)(long)n, b, &disksim_iomap_mod);
	 ...
	 if (tracemap2[i]) {
	    if (tracemap2[i] < 1) {
	       ...
	       if (temp->blkno % tracemap2[i]) {
	          fprintf(stderr, "Small sector size disk using odd sector number: %ld\n", temp->blkno);
	          exit(1);
	       }
/*
...
event * io_get_next_external_event (FILE *iotracefile)
{
   ...
   if (temp) {
	 ...
	 fprintf(stderr, "devno %d, blkno %ld, bcount %d, flags %d\n", temp->devno, temp->blkno, temp->bcount, temp->flags);
         exit(1);
      }
      if (tracemappings) {
...
void io_printstats(int sub_status)
{
   int i;
   int cnt = 0;
   char prefix[80];
   ...
   iodriver_printstats(sub_status);
   device_printstats();
   if(sub_status)
	return;
