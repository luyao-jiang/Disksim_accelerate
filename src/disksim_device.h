...
struct device_header {
  ...
  void    (*bus_ownership_grant)(int devno, 
				 ...
				 int busno, 
				 double arbdelay);
  void 	  (*notify_trace_done)();

  // KJ: function pointer to SSD model
  int     (*fast_forward_simulation)(int cur_repeat_trace_count);
  double     (*calculate_distance)(FILE *prev_snapshot, FILE *curr_snapshot);
  // KJ: for clustering
  int (*get_subreq_elem) (int devno, long int blkno);
  int (*get_subreq_count) (int devno, long int blkno, int count);
...
void	device_notify_trace_done ();

// KJ: used in disksim_iotrace.c
int device_fast_forward_simulation(int cur_repeat_trace_count);
double device_calculate_distance(FILE *prev_snapshot, FILE *curr_snapshot);
// KJ: needed to move part of SSD mapping logic to disksim layer
int device_get_subreq_elem(int devno, long int blkno);
int device_get_subreq_count(int devno, long int blkno, int count);
