...
void device_add(struct device_header *d, int ldevno) {
  ...
  if (c == disksim->deviceinfo->devs_len) {
  ...
  disksim->deviceinfo->devices = realloc(disksim->deviceinfo->devices, 
        newlen*sizeof(struct device_header *));
    bzero(disksim->deviceinfo->devices + c, zerocnt * sizeof(struct device_header *));

  disksim->deviceinfo->devs_len = newlen;
  }

  ddbg_assert(c < disksim->deviceinfo->devs_len);
...
INLINE void device_notify_trace_done()
{
	return disksim->deviceinfo->devices[0]->notify_trace_done();
}


/* KJ: this is fast-forward simulation */
/* takes the current trace count as the argument and returns the trace_repeat rounds in fast-forward simulation */
INLINE int device_fast_forward_simulation(int cur_repeat_trace_count)
{
	return disksim->deviceinfo->devices[0]->fast_forward_simulation(cur_repeat_trace_count);
}

INLINE double device_calculate_distance(FILE *prev_snapshot, FILE *curr_snapshot)
{
	return disksim->deviceinfo->devices[0]->calculate_distance(prev_snapshot, curr_snapshot);
}

// KJ: for clustering
INLINE int device_get_subreq_elem(int devno, long int blkno)
{
	return disksim->deviceinfo->devices[0]->get_subreq_elem(devno, blkno);
}

// KJ:
INLINE int device_get_subreq_count(int devno, long int blkno, int count)
{
	return disksim->deviceinfo->devices[0]->get_subreq_count(devno, blkno, count);
}
