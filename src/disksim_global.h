...
// KJ: for fast forward
struct prev_trace_record{
  struct prev_trace_entry *head;
  int index;
  int curr_len;
};

struct prev_trace_entry{
  int pageno;
  int blkno;
  int elemno;
  double access_time;
...
typedef struct disksim {
   ...
   double checkpoint_starttime;
   int	  max_block_per_device_in_trace[100];
   char   snapshot_output_path[500];
   int	  no_of_devices_in_trace;
   double simtime;
   ...
   FILE * parfile;
   FILE * iotracefile;
   FILE * master_trace_file;
   FILE * statdeffile;
   FILE * outputfile;
   FILE * snapshotfile;
   FILE * ta_snapshotfile;
   FILE * ds_snapshotfile;
   char ip_snapshotfilename[500];
   char * curr_snapshot_output_file;
 //  int    snapshot_startindex;
   // CHANGED: just need to keep track of the index of the current snapshot that is about to be dumped;
   // KJ: add some new args here
   int    repeat_trace;
   int    snapshot_currindex;
   char * simulation_flag;
   int checkpoint_enabled;
   int warmup_rounds;
   double    threshold;
   int    detailed_simulation_duration;
   int    fast_forward_strategy;
   int    fast_forward_duration;
   // record here so that when it reaches the preset fast forward duration, dump the info 
   int    cur_detailed_trace_count;
   int	  cur_repeat;
   int    is_past_threshold;
   // for duplicate previous detailed round ff simulation
   // the cost of reading and writing is too large
 //  FILE * prev_trace_file;
   struct prev_trace_record *curr_record;
   // TODO: remove debug only
 //  int debug_only;
  // KJ: to capture duration in fastforward mode
   double duration;
   double start_time_of_next_round;
   // KJ: cluster array
   int ** cluster;
   int num_of_cluster;
   int * cluster_size;
   // for stress distribution sampling
   int window_size;
   int num_of_spatial_bucket;
   int num_of_temporal_bucket;

   // KJ: matrix that keeps track of the spatial and temporal distribution
   int *** stress_dist_matrix;

   #ifdef DEBUG
   int tot_stress_before;
   int tot_stress;
   #endif
   ...
   char   outiosfilename[500];
   char   checkpointfilename[256];
   // KJ: ssd checkpoint file
   char   device_checkpoint_prefix[500];
   // KJ: snapshot startindex might not be 1 cuz of checkpoint
   int snapshot_startindex;
   // KJ: add prev_trace_file_name here
 //  char   prev_trace_file_name[500];
...
#define checkpoint_starttime	 (disksim->checkpoint_starttime)
#define statdeffile      (disksim->statdeffile)
#define outputfile       (disksim->outputfile)
#define snapshotfile	   (disksim->snapshotfile)
#define ta_snapshotfile  (disksim->ta_snapshotfile)
#define ip_snapshotfilename (disksim->ip_snapshotfilename)
#define ds_snapshotfile  (disksim->ds_snapshotfile)
#define master_trace_file (disksim->master_trace_file)
...
//extern void notify_trace_done(); //Notify that the current trace id done.
// KJ: set_prev_trace_file
void set_prev_trace_file(struct prev_trace_record * curr_record);
// KJ: initialize stress dist matrix by set every element to 0
void initialize_stress_dist_matrix();
