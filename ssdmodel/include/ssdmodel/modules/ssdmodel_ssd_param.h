...
typedef enum {
   ...
   SSDMODEL_SSD_CHECKPOINT_TIME,
   SSDMODEL_SSD_REFRESH_INTERVAL,
   SSDMODEL_SSD_REFRESH_POLICY,
   SSDMODEL_SSD_VIRTUAL_RETENTION_PERIOD,
   SSDMODEL_SSD_MINIMUM_RETENTION_PERIOD
} ssdmodel_ssd_param_t;

#define SSDMODEL_SSD_MAX_PARAM		SSDMODEL_SSD_MINIMUM_RETENTION_PERIOD
...
static struct lp_varspec ssdmodel_ssd_params [] = {
   ...
   {"Checkpoint time", D, 1 },
   {"Refresh interval", D, 1 },
   {"Refresh policy", I, 1 },
   {"Virtual retention period", D, 1 },
   {"Minimum retention period", D, 1 },
   {0,0,0}
};
