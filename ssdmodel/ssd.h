...
//Supported refresh policies - as of now FCFS only.
#define SSD_REFRESH_NONE            0
#define SSD_FCFS_REFRESH            1
#define SSD_EDF_REFRESH             2
#define SSD_MAX_REFRESH_POLICY      5

#define SSD_MAX_ELEMENTS            64
#define SSD_MAX_PLANES_PER_ELEM     16
#define SSD_MAX_PARUNITS_PER_ELEM   SSD_MAX_PLANES_PER_ELEM
#define SSD_MAX_ELEMS_PER_GANG      SSD_MAX_ELEMENTS
// if you're changing the above, do change the following bits too
...
#define SSD_MIN_LIFETIME_TO_ADD_TO_REFRESH_QUEUE (18 * MONTH)
...
typedef struct _ssd_element_stat {
    ...
    int num_refresh;                // no of times refresh has been invoked on this element
    ...
    double tot_refresh_time;        // sum of refresh time.

    long int tot_physical_stresses;   // total stress applied by the model for all pages in the element.
    long int tot_logical_stresses;    // total stress applied by the workload for all pages in the element.
    double tot_recovery_period;     // total recovery period for all pages in the element (should this be equal to duration of trace?)
} ssd_element_stat;
...
typedef enum {
  ...
  SSD_BLOCK_SEALED,
  SSD_BLOCK_DEAD,
  SSD_BLOCK_RETENTION_LIMIT,
} SSD_BLOCK_STATES;


typedef struct _page_metadata {
  int     lpn;
  int     logical_stresses;
  int	    physical_stresses;
  int     stress_increment;
  double  time_of_last_stress;
  double  retention_period; //in seconds
  double  eqn_cycle;
	double	recovery_period_total;
  double  delvth;
}ssd_page_metadata;
...
typedef struct _block_metadata {
    ...
    int         elem_num;               // the element to which this block belong to - vb.
	
    ssd_page_metadata *page;                  // size of the array = pages_per_block
    ...
    int         state;                  // state of the block
                                        ...
                                        // DEAD - The block is dead.


    unsigned int    bsn;                // block sequence number (version number for blocks)


  //Summary of page level statistics stored in block metadata.
  int    			total_pages_written;    // Used to calculate write amplification factor.
	ssd_page_metadata *least_retention_page;
	void*				entry_in_refresh_queue; //Use this ptr to random access in the queue.

  long int physical_stresses;   // total stress applied by the model for all pages in this block
  long int logical_stresses;    // total stress applied by the workload for all pages in this block
...
typedef struct _plane_metadata {
    ...
    int dead_blocks;                // number of dead blocks in the plane.
    ...
    int clean_in_progress;          // a flag that is set to 1 when some cleaning or refresh is
                                    // going on in this plane
    int clean_in_block;             // which block is being cleaned/refresh'ed?
    ...
    int num_refreshes;              // number of times refresh was invoked on this plane.
} plane_metadata;
...
typedef struct _ssd_timing_params {
    ...
    double	checkpoint_time;			      //how often do you want to checkpoint the status of blocks(in hours).

		double	refresh_interval;			      //refresh rate specified during configuration.

    int     refresh_policy;             //refresh policy.

    double  refresh_service_time;       //This is the time it takes to move all pages in a block to another block and clean this block.
		
		double  min_retention_period;       //Lower threshold on the retention period. - set in ssd_init.c

    double  virtual_retention_period;   //Our definition of when blocks become no longer usable. (input is in hours).
} ssd_timing_params;
...
typedef struct ssd {
   ...
   ssd_stat_t stat;
	 ...
	 struct _listnode *refresh_queue;
...
void 	add_final_idle_time (ssd_t *s);
double 	ssd_compute_avg_idle_time(int elem_num,ssd_t *currdisk);
...
void 	ssd_update_stress_info_block(block_metadata *metadata, double time, ssd_t *s);
void 	ssd_update_stress_info(ssd_page_metadata *page_metadata,block_metadata *metadata, double time, ssd_t *s);
void update_lifetime(ssd_page_metadata *page_metadata, block_metadata *metadata, double recovery_period, ssd_t *s);

// KJ: FIXME: seems not needed
int ssd_get_subreq_elem(int devno, long int blkno);
int ssd_get_subreq_count(int devno, long int blkno, int count);
