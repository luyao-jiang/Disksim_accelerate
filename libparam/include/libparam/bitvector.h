...
/* turn on all y bits of x */
#define BIT_SETALL(x,y) { int i; for (i = 0; i < BVSIZE(y); ++i) x[i] = 0xFF; }

/* turn off the yth bit of x */
#define BIT_RESET(x,y) x[y / 8] &= ~(1 << (y % 8))

/* turn off all y bits of x */
#define BIT_RESETALL(x,y) { int i; for (i = 0; i < BVSIZE(y); ++i) x[i] = 0x00; }
